...
class EmailVerificationController {
  ...
  void initializeController() {
    //tilak authRepo.sendEmailVerification(context);
  ...
  void checkEmailVerified() async {
    //tilak await auth.currentUser?.reload().then((value)
    {
      if(mounted){

        /// Calls the authentication repository's FirebaseAuth user to check whether email is verified or not
        // tilak verified.value = auth.currentUser!.emailVerified;
        verified.value = true;
        print('tilak ' + verified.value.toString());
        if(verified.value) {
